...
Route::middleware('auth')->prefix('assistant')->name('web.assistant.')->group(function () {
    ...
    // Cập nhật một đợt đồ án (PUT)
    Route::put('/thesis/rounds/{project_term}', [ProjectTermsController::class, 'update'])->name('rounds.update');
    // Xóa một đợt đồ án  
...
Route::middleware(['web','auth'])->prefix('admin')->name('web.admin.')->group(function () {
    ...
    // PUT route to update a student (used by admin modal)
    Route::put('/manage_students/{student}', [WebStudentController::class, 'update'])->name('manage_students.update');
